...
export default function History() {
    ...
    const init = async () => {
      try {
        ...
        if (user.role === "admin") {
          ...
          if (initialDevice) {
            const historyData = await getSensorHistoryRange(initialDevice, { days: 3 });
            setRecords(historyData);
          } else {
            setRecords([]);
          }
        } else {
          const assignedDevice = user.deviceId || "";
          setDevices(assignedDevice ? [assignedDevice] : []);
          setSelectedDevice(assignedDevice);

          if (assignedDevice) {
            const historyData = await getSensorHistoryRange(assignedDevice, { days: 3 });
            setRecords(historyData);
          } else {
            setRecords([]);
          }
  ...
  useEffect(() => {
    if (!user || !selectedDevice || user.role !== "admin") return;
    ...
    <Box sx={{ minHeight: "100vh", background: "#040d08", color: "#e8f5e9" }}>
      ...
      >
        <Box>
          ...
          >
            Last 3 days of sensor and relay history only
          </Typography>
        </Box>
