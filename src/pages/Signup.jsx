...
export default function Signup() {
  ...
  return (
    ...
    >
      ...
      {/* Background glows */}
      <Box sx={{ position:"absolute",top:"5%",right:"8%",width:350,height:350,borderRadius:"50%",background:"radial-gradient(circle,rgba(45,106,79,0.16) 0%,transparent 70%)",pointerEvents:"none" }} />
      <Box sx={{ position:"absolute",bottom:"5%",left:"5%",width:280,height:280,borderRadius:"50%",background:"radial-gradient(circle,rgba(52,211,153,0.09) 0%,transparent 70%)",pointerEvents:"none" }} />
      ...
      <Box
        ...
        sx={{
          ...
          boxShadow: "0 32px 80px rgba(0,0,0,0.6), inset 0 1px 0 rgba(74,222,128,0.08)",
          overflow: "hidden",
        }}
      >
        {/* Top accent */}
        ...
        <Box sx={{ p: { xs: 4, md: 5 } }}>
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={0}>
            <Box sx={{ display:"flex",alignItems:"center",gap:1,mb:5 }}>
              <Box sx={{ width:32,height:32,borderRadius:"8px",background:"linear-gradient(135deg,#2d6a4f,#4ade80)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16 }}>
                🌿
              </Box>
              <Typography sx={{ fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontWeight:600,color:"#e8f5e9" }}>
                SmartCultivation
              </Typography>
            </Box>
          </motion.div>
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={1}>
            <Typography sx={{ fontFamily:"'Cormorant Garamond',serif",fontSize:"2rem",fontWeight:300,color:"#e8f5e9",lineHeight:1.1,mb:0.8 }}>
              Create Account
            </Typography>
            <Typography sx={{ fontSize:13.5,color:"rgba(232,245,233,0.4)",mb:4 }}>
              Join to monitor your cultivation chamber
            </Typography>
          </motion.div>

          {/* Role Toggle */}
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={2}>
            <Typography sx={{ fontSize:11,letterSpacing:2,color:"rgba(232,245,233,0.4)",textTransform:"uppercase",fontFamily:"'JetBrains Mono',monospace",mb:1.5 }}>
              Account Type
            </Typography>
            <ToggleButtonGroup
              value={role}
              ...
              {[
                ...
                <ToggleButton
                  ...
                  sx={{
                    ...
                    boxShadow: role === r.val ? "0 0 14px rgba(74,222,128,0.12)" : "none",
                ...
                >
                  <Typography sx={{ fontSize:14, fontWeight:500, color:"inherit", textTransform:"none" }}>
                    {r.label}
                  </Typography>
                  <Typography sx={{ fontSize:11, opacity:0.65, textTransform:"none", color:"inherit" }}>
                    {r.desc}
                  </Typography>
                </ToggleButton>
              ))}
          ...
          {error && (
            <motion.div initial={{ opacity:0, y:-8 }} animate={{ opacity:1, y:0 }}>
              <Alert
                severity="error"
                sx={{ mb:2.5,borderRadius:"8px",background:"rgba(248,113,113,0.08)",color:"#fca5a5",border:"1px solid rgba(248,113,113,0.2)","& .MuiAlert-icon":{ color:"#f87171" } }}
              >
                {error}
              </Alert>
            </motion.div>
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={7}>
            <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              <Button
                ...
                disabled={loading}
                sx={{
                  py: 1.6, fontSize: 14, borderRadius: "8px", fontWeight: 500,
                  background: "linear-gradient(135deg,#2d6a4f,#4ade80)",
                  color: "#040d08",
                  boxShadow: "0 8px 24px rgba(74,222,128,0.25)",
                  mb: 3,
                  transition: "all 0.3s",
                  "&:hover": { boxShadow: "0 12px 32px rgba(74,222,128,0.45)" },
            ...
            <Box sx={{ pt:3, borderTop:"1px solid rgba(74,222,128,0.08)", textAlign:"center" }}>
              <Typography sx={{ fontSize:13.5, color:"rgba(232,245,233,0.35)" }}>
                ...
                <Box
                  ...
                  sx={{ color:"#4ade80", cursor:"pointer", fontWeight:500, "&:hover":{ textDecoration:"underline" } }}
                >
                  Sign In
                </Box>
              </Typography>
              <Box
                component="span"
                onClick={() => navigate("/")}
                sx={{ display:"inline-flex", alignItems:"center", gap:0.5, mt:2, fontSize:12, color:"rgba(232,245,233,0.25)", cursor:"pointer", "&:hover":{ color:"rgba(232,245,233,0.5)" } }}
              >
                ← Back to Home
              </Box>
            </Box>
