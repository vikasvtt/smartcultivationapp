...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    const ok = await login(email, password); // ✅ no fake delay
    setLoading(false);
  ...
  return (
    <Box sx={{
      ...
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500&family=Cormorant+Garamond:ital,wght@0,300;0,600;1,300&display=swap');
        *{box-sizing:border-box;font-family:'DM Sans',sans-serif}
        @keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.4;transform:scale(.8)}}
      `}</style>

      <Box sx={{ position:"absolute",top:"10%",left:"5%",width:{xs:200,md:400},height:{xs:200,md:400},borderRadius:"50%",background:"radial-gradient(circle,rgba(45,106,79,0.18) 0%,transparent 70%)",pointerEvents:"none" }} />
      <Box sx={{ position:"absolute",bottom:"10%",right:"5%",width:{xs:150,md:300},height:{xs:150,md:300},borderRadius:"50%",background:"radial-gradient(circle,rgba(52,211,153,0.1) 0%,transparent 70%)",pointerEvents:"none" }} />
      ...
      <Box component={motion.div} initial={{ opacity:0,scale:0.95 }} animate={{ opacity:1,scale:1 }} transition={{ duration:0.6,ease:[0.22,1,0.36,1] }}
        sx={{ width:"100%",maxWidth:440,borderRadius:"18px",border:"1px solid rgba(74,222,128,0.14)",background:"rgba(8,15,10,0.85)",backdropFilter:"blur(24px)",boxShadow:"0 32px 80px rgba(0,0,0,0.6),inset 0 1px 0 rgba(74,222,128,0.08)",overflow:"hidden" }}>
        ...
        <Box sx={{ p:{xs:3,md:5} }}>
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={1}>
            <Typography sx={{ fontFamily:"'Cormorant Garamond',serif",fontSize:{xs:"1.7rem",md:"2rem"},fontWeight:300,color:"#e8f5e9",lineHeight:1.1,mb:0.8 }}>Welcome back</Typography>
            <Typography sx={{ fontSize:13.5,color:"rgba(232,245,233,0.4)",mb:3 }}>Sign in to your cultivation dashboard</Typography>
          </motion.div>
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" custom={4}>
            <motion.div whileHover={{ scale:1.02 }} whileTap={{ scale:0.98 }}>
              <Button fullWidth onClick={handleLogin} disabled={loading}
                ...
                {loading ? <CircularProgress size={18} sx={{ color:"#040d08" }} /> : "Sign In"}
              </Button>
            </motion.div>
          </motion.div>
